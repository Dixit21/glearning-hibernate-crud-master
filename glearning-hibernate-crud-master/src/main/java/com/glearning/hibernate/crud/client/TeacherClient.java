...
import com.glearning.hibernate.crud.model.Subject;
...
public class TeacherClient {
	...
	public static void main(String[] args) {
		...
		/*
		 * client.insertTeacher(); //fetch all the teachers client.fetchTeachers();
		 * 
		 * //fetch teacher by teacher id client.fetchTeacherByTeacherId(1);
		 * 
		 * //delete the teacher record from the db based on id
		 * client.deleteTeacherByTeacherId(1);
		 */
		
		Teacher bhuvan = client.insertTeacher();
		client.insertSubjects(bhuvan);
		
	}

	private void insertSubjects(Teacher bhuvan) {
		
		Subject maths = new Subject("Maths");
		Subject science = new Subject("Sience");
		
		bhuvan.addSubject(maths);
		bhuvan.addSubject(science);
		
		TeacherDetails bhuvanTeacherDetails = new TeacherDetails("888-898-8777", LocalDate.now(), "Chennai");
		
		this.teacherService.insertTeacherRecord(bhuvan, bhuvanTeacherDetails);
